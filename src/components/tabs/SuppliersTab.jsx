...
export default function SuppliersTab() {
  ...
  const handleOpenStatement = (supplier) => {
    ...
    setToDate(toDateStr);
    loadStatement(supplier.id, fromDateStr, toDateStr);
    ...
    <Box>
      ...
      >
        <Alert severity="info" sx={{ mb: 3, fontSize: '1rem', display: 'flex', gap: 0.5, alignItems: 'center' }}>
          ...
          <Box component="span" sx={{ fontWeight: 700 }}>{fmt(selectedSupplier?.balance)} {baseCurrencyInfo.symbol}</Box>
        </Alert>
      ...
      {selectedBatch && (
        <UnifiedFormDialog
          ...
          title={`تفاصيل الدفعة #${selectedBatch.id}`}
          submitText="إغلاق"
          cancelText=""
          maxWidth="md"
        >
