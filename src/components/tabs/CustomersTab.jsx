...
export default function CustomersTab() {
  ...
  const loadData = () => {
    try {
      ...
      setError('فشل تحميل الزبائن: ' + (e?.message || String(e)));
    }
  };
  ...
  const handleOpenStatement = (customer) => {
    ...
    setToDate(toDateStr);
    loadStatement(customer.id, fromDateStr, toDateStr);
  ...
  return (
    <Box>
      ...
      <UnifiedFormDialog
        ...
        loading={loading}
      >
        <Alert severity="info" sx={{ mb: 3, fontSize: '1rem', display: 'flex', gap: 0.5, alignItems: 'center' }}>
          <Box component="span">الرصيد الحالي:</Box>
          <Box component="span" sx={{ fontWeight: 700 }}>{fmt(selectedCustomer?.balance)} {baseCurrencyInfo.symbol}</Box>
        </Alert>
