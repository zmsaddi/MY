...
export default function InventoryTab() {
  ...
  const refreshAll = () => {
    // Clean up empty batches before loading data
    const pruneResult = pruneEmptyBatches();
    if (pruneResult.count > 0) {
      console.log(`✓ Pruned ${pruneResult.count} empty batches`);
    }

    const all = getAllSheets();
    const fullSheets = all.filter(s => !s.is_remnant && s.total_quantity > 0);
    const remnantSheets = all.filter(s => s.is_remnant && s.total_quantity > 0);
  ...
  // Auto-generate code when dimensions/grade/finish change (for regular sheets)
  useEffect(() => {
  ...
  // Auto-fill remnant properties from parent sheet when parent is selected
  useEffect(() => {
    if (remnantForm.piece_source === 'from_sheet_cutting' && remnantForm.parent_sheet_id) {
      const allSheets = [...sheets, ...remnants]; // Include both full sheets and remnants
      const parentSheet = allSheets.find(s => s.id === Number(remnantForm.parent_sheet_id));
      ...
      if (parentSheet) {
        setRemnantForm(prev => ({
          ...
          thickness_mm: parentSheet.thickness_mm,
          // Inherit weight_per_cm2 if available, otherwise weight_per_sheet_kg
          weight_per_sheet_kg: parentSheet.weight_per_cm2 ? '' : (parentSheet.weight_per_sheet_kg || '')
        }));

        // Update grades and finishes for the selected metal type
        if (parentSheet.metal_type_id) {
          setGrades(getGrades(parentSheet.metal_type_id, true));
          setFinishes(getFinishes(parentSheet.metal_type_id, true));
        }
      }
    }
  }, [remnantForm.parent_sheet_id, remnantForm.piece_source, sheets, remnants]);

  // Auto-generate code when dimensions/grade/finish change (for remnants)
  useEffect(() => {
    if (autoGenerateCode && remnantForm.metal_type_id && remnantForm.length_mm && remnantForm.width_mm && remnantForm.thickness_mm) {
      const generated = generateSheetCode(
        ...
        true // isRemnant - THIS IS THE KEY DIFFERENCE!
      );
  ...
  const filteredRemnants = useMemo(() => {
    ...
    if (remnantFilterParentSheet) {
      rows = rows.filter(s => {
        ...
        // Search in both full sheets and remnants (since parent could be either)
        const allSheets = [...sheets, ...remnants];
        const parentSheet = allSheets.find(sh => sh.id === s.parent_sheet_id);
  ...
  const validateBatchForm = () => {
    ...
    if (weightPerSheet <= 0 && totalWeight <= 0) {
      newErrors.weight_per_sheet = 'يجب إدخال الوزن (لكل قطعة أو الإجمالي)';
      newErrors.total_weight = 'يجب إدخال الوزن (لكل قطعة أو الإجمالي)';
    }
  ...
  const handleActualSaveRemnant = async () => {
    ...
    setLoading(true);
    try {
      // Get parent sheet if exists to inherit weight_per_cm2
      let weightPerCm2 = null;
      if (remnantForm.piece_source === 'from_sheet_cutting' && remnantForm.parent_sheet_id) {
        const allSheets = [...sheets, ...remnants];
        const parentSheet = allSheets.find(s => s.id === Number(remnantForm.parent_sheet_id));
        if (parentSheet && parentSheet.weight_per_cm2) {
          weightPerCm2 = parentSheet.weight_per_cm2;
        }
      }
      ...
      const sheetData = {
        ...
        weight_per_cm2: weightPerCm2, // Pass inherited weight_per_cm2 to database
        is_remnant: true,
  ...
  const handleActualSaveBatch = async () => {
    ...
    if (!existingBatchForm.batch_weight_kg || Number(existingBatchForm.batch_weight_kg) <= 0) {
      newErrors.batch_weight_kg = 'وزن الدفعة مطلوب ويجب أن يكون أكبر من صفر';
    }
  ...
  const handleDeleteSheet = () => {
    openConfirm('deleteSheet', selectedSheet, handleActualDeleteSheet);
  };
  ...
  // Check if a sheet can be deleted (no sales, no child remnants)
  const canDeleteSheet = (sheet) => {
    if (!sheet) return false;

    // Check for child remnants
    const allSheets = [...sheets, ...remnants];
    const hasChildRemnants = allSheets.some(s => s.parent_sheet_id === sheet.id);

    // Note: We can't check for sales here without querying the database
    // The deleteSheet function in the database will handle that check
    return !hasChildRemnants;
  };
    ...
    <Box>
            ...
            <AccordionDetails>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  ...
                  {(!batchForm.weight_per_sheet && !batchForm.total_weight) && (
                    <Alert severity="warning" sx={{ mt: 1, fontSize: '0.875rem' }}>
                      ⚠️ بدون إدخال الوزن، لن يتمكن النظام من حساب الوزن تلقائياً عند البيع
                    </Alert>
                  )}
                </Grid>
                        ...
                        return (
                          <>
                            <Grid item xs={12} md={4}>
                              ...
                              >
                                <Typography component="span" sx={{ fontWeight: 700, fontSize: '1.1rem' }}>السعر لكل قطعة:</Typography>
                                <Typography component="span" sx={{ fontWeight: 600, fontSize: '1.1rem' }}>{fmt(pricePerSheet)} {baseCurrencyInfo.symbol}</Typography>
                              </Typography>
                            </Grid>
                            <Grid item xs={12} md={4}>
                              <Typography
                              ...
                              >
                                <Typography component="span" sx={{ fontWeight: 700, fontSize: '1.1rem' }}>السعر لكل كيلو:</Typography>
                                <Typography component="span" sx={{ fontWeight: 600, fontSize: '1.1rem' }}>{fmt(calculatedPricePerKg)} {baseCurrencyInfo.symbol}</Typography>
                              </Typography>
                            </Grid>
                            <Grid item xs={12} md={4}>
                              <Typography
                              ...
                              >
                                <Typography component="span" sx={{ fontWeight: 700, fontSize: '1.1rem' }}>التكلفة الإجمالية:</Typography>
                                <Typography component="span" sx={{ fontWeight: 600, fontSize: '1.1rem' }}>{fmt(calculatedTotalCost)} {baseCurrencyInfo.symbol}</Typography>
                              </Typography>
                            </Grid>
                          </>
                        );
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} md={6}>
                  <UnifiedFormField
                    ...
                    name="supplier_id"
                    select
                    disabled={remnantForm.piece_source === 'from_sheet_cutting'}
                    helperText={remnantForm.piece_source === 'from_sheet_cutting' ? 'البواقي من القص الداخلي لا تحتاج مورد' : ''}
                  >
                ...
                <Grid item xs={12}>
                  <WeightPriceEntry
                    mode="weight"
                    weightMode={remnantBatchForm.weight_input_mode || 'per_sheet'}
                    label="الوزن"
                    value={remnantBatchForm.weight_per_sheet || ''}
                    totalWeight={remnantBatchForm.batch_weight_kg || ''}
                    quantity={remnantBatchForm.quantity || ''}
                    onChange={(field, value) => {
                      if (field === 'weight_mode') {
                        setRemnantBatchForm({ ...remnantBatchForm, weight_input_mode: value });
                      } else if (field === 'weight_per_sheet') {
                        setRemnantBatchForm({ ...remnantBatchForm, weight_per_sheet: value });
                        if (value && remnantBatchForm.quantity) {
                          const total = Number(value) * Number(remnantBatchForm.quantity);
                          setRemnantBatchForm(prev => ({ ...prev, batch_weight_kg: total.toFixed(3) }));
                        }
                        // Also update remnantForm.weight_per_sheet_kg
                        setRemnantForm(prev => ({ ...prev, weight_per_sheet_kg: value }));
                      } else if (field === 'total_weight') {
                        setRemnantBatchForm({ ...remnantBatchForm, batch_weight_kg: value });
                        if (value && remnantBatchForm.quantity) {
                          const perSheet = Number(value) / Number(remnantBatchForm.quantity);
                          setRemnantBatchForm(prev => ({ ...prev, weight_per_sheet: perSheet.toFixed(3) }));
                          setRemnantForm(prev => ({ ...prev, weight_per_sheet_kg: perSheet.toFixed(3) }));
                        }
                      }
                    }}
                    currencySymbol={baseCurrencyInfo.symbol}
                  />
                  {remnantBatchErrors.weight && (
                    <Typography color="error" variant="caption" sx={{ mt: 0.5, display: 'block' }}>
                      {remnantBatchErrors.weight}
                    </Typography>
                  )}
                ...
                <Grid item xs={12}>
                  <WeightPriceEntry
                    ...
                    quantity={remnantBatchForm.quantity || ''}
                    weight={remnantBatchForm.batch_weight_kg || (Number(remnantBatchForm.weight_per_sheet || 0) * Number(remnantBatchForm.quantity || 0))}
                    onChange={(field, value) => {
                      if (field === 'pricing_mode') {
                ...
                <Grid item xs={12}>
                  ...
                  >
                    <Typography variant="body1" sx={{ mb: 2, fontWeight: 600, fontSize: '1.05rem' }}>
                      ℹ️ الأسعار للمرجع فقط ولا تؤثر على حسابات الموردين (البقايا مخزون داخلي)
                    </Typography>
                    <Grid container spacing={2}>
                      {(() => {
                        const qty = Number(remnantBatchForm.quantity) || 0;
                        const weightPerSheet = Number(remnantBatchForm.weight_per_sheet) || 0;
                        const totalWeight = Number(remnantBatchForm.batch_weight_kg) || (qty * weightPerSheet);
                        const pricePerKg = Number(remnantBatchForm.price_per_kg) || 0;
                        const totalCost = Number(remnantBatchForm.total_cost) || 0;

                        let pricePerSheet = 0;
                        let calculatedTotalCost = 0;
                        let calculatedPricePerKg = 0;

                        if (remnantBatchForm.pricing_mode === 'per_kg') {
                          calculatedTotalCost = totalWeight * pricePerKg;
                          pricePerSheet = weightPerSheet * pricePerKg;
                          calculatedPricePerKg = pricePerKg;
                        } else {
      ...
      >
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          ...
          {!canDeleteSheet(selectedSheet) && selectedSheet && (
            <Tooltip title="لا يمكن حذف الصفيحة لأن لديها بواقي مرتبطة أو مبيعات">
              <span>
                <Button
                  variant="outlined"
                  color="error"
      ...
      <UnifiedFormDialog
        ...
        submitText={isEditingBatch ? 'حفظ التعديلات' : 'تم'}
        {...(isEditingBatch && { cancelText: 'إلغاء' })}
        loading={loading}
        maxWidth="sm"
      >
        <Box sx={{ mb: 2 }}>
