...
import { db, tx, saveDatabase, safe, round2, lastId, currencyHelpers, generateInvoiceNumber, getCurrentUser, hasColumn } from './core.js';
...
import { getBatchesBySheetId, recordInventoryMovement, pruneEmptyBatches, deleteSheet } from './inventory.js';
...
export function deleteSale(saleId) {
  try {
    ...
    // First, delete remnants created from this sale (if any)
    // Check if created_from_sale_id column exists
    const hasCreatedFromSaleId = hasColumn('sheets', 'created_from_sale_id');
    if (hasCreatedFromSaleId) {
      const remnantStmt = db.prepare('SELECT id FROM sheets WHERE created_from_sale_id = ?');
      remnantStmt.bind([saleId]);

      const remnantIds = [];
      while (remnantStmt.step()) {
        const row = remnantStmt.getAsObject();
        remnantIds.push(row.id);
      }
      remnantStmt.free();

      // Delete each remnant using the deleteSheet function (imported at top)
      for (const remnantId of remnantIds) {
        const deleteResult = deleteSheet(remnantId);
        if (!deleteResult.success) {
          console.warn(`Failed to delete remnant ${remnantId}:`, deleteResult.error);
        }
      }
    }
