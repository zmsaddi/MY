...
export function runMigrations() {
  ...
  if (!hasColumn('sheets', 'weight_per_cm2')) {
    ...
    console.log('✓ Added weight_per_cm2 column to sheets table for auto-calculation');
    ...
    try {
      ...
      updateStmt.free();
      if (updated > 0) {
        console.log(`✓ Backfilled weight_per_cm2 for ${updated} existing sheets`);
      }
    } catch (e) {
  ...
  if (!hasColumn('sheets', 'created_from_sale_id')) {
    ...
    console.log('✓ Added created_from_sale_id column to sheets table for remnant tracking');
  }
...
async function addAuditColumns() {
  ...
  if (localStorage.getItem(migrationFlag) === 'true') {
    console.log('✓ Audit columns migration already completed (cached in localStorage)');
    return;
  }
  ...
  try {
    ...
    if (migrationCompleted) {
      ...
      console.log('✓ Audit columns migration already completed (cached in IndexedDB)');
      return;
    }
  } catch (e) {
  ...
  if (columnsAdded) {
    ...
    try {
      ...
      console.log('✓ Audit columns migration completed and marked as done');
    } catch (e) {
      console.log('✓ Audit columns migration completed and marked as done in localStorage');
    }

    // Log completion without reload - app continues normally
    console.log('✅ Database migration complete. Application will continue without reload.');
    ...
    try {
      const { indexedDBManager } = await import('./indexedDBManager.js');
      await indexedDBManager.setSetting(migrationFlag, true);
      console.log('✓ All audit columns already exist, migration marked as completed');
    } catch (e) {
      console.log('✓ All audit columns already exist, migration marked as completed in localStorage');
    }
  }
}
