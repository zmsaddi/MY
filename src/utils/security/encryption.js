...
class DatabaseEncryption {
   ...
   * This prevents database corruption when sessionStorage is cleared
   */
  initializeKey() {
    // First try to get the persistent key from localStorage
    const persistentKey = localStorage.getItem('_db_enc_key_v2');

    if (persistentKey) {
      this.encryptionKey = persistentKey;
      // Also set in sessionStorage for faster access
      sessionStorage.setItem('_db_enc_key', persistentKey);
    } else {
      // Check if there's an old key in sessionStorage (for migration)
      const sessionKey = sessionStorage.getItem('_db_enc_key');

      if (sessionKey) {
        // Migrate the key to localStorage
        this.encryptionKey = sessionKey;
        localStorage.setItem('_db_enc_key_v2', sessionKey);
      } else {
        // Generate a new key and store it persistently
        this.encryptionKey = this.generateKey();
        localStorage.setItem('_db_enc_key_v2', this.encryptionKey);
  ...
  async encrypt(data) {
    ...
    try {
      ...
      // Convert to base64 - fixed to handle large data without stack overflow
      let binary = '';
  ...
  async decrypt(encryptedData) {
    ...
    try {
      ...
      // Don't fall back to XOR for AES-encrypted data - it will produce garbage
      // This is likely due to a key mismatch (e.g., data encrypted with old key)
      console.error('AES Decryption failed:', error);

      // Check if this looks like AES data (has proper length for IV + data)
      try {
        const testDecode = atob(encryptedData);
        if (testDecode.length > 12) {
          // This is likely AES data that can't be decrypted (wrong key)
          throw new Error('Unable to decrypt data - encryption key mismatch. Data may have been encrypted with a different key.');
        }
      } catch (e) {
        // Not even valid base64, definitely corrupted
        throw new Error('Data corruption detected - invalid encrypted data format.');
      }

      // Only use deobfuscation for non-AES data (legacy fallback)
      console.warn('Attempting XOR deobfuscation for legacy data');
      return this.deobfuscate(encryptedData);
    }
  }
