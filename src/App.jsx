...
function App() {
  ...
  useEffect(() => {
    ...
    const checkInactivity = () => {
      if (isSessionExpired()) {
        ...
        // Move session clear work off the main thread
        setTimeout(() => {
          clearSession();
        }, 0);

        // Wrap all expensive state changes in startTransition to prevent Suspense violations
        startTransition(() => {
  ...
  const handleLogout = () => {
    ...
    // Move session clear work off the click stack
    setTimeout(() => {
      clearSession();
    }, 0);
  ...
  const handleTabChange = (index) => {
    setMobileOpen(false); // Close drawer immediately
    // Wrap lazy tab loading in transition to prevent Suspense violations
    startTransition(() => {
  ...
  // Wrap tab switches in startTransition to prevent Suspense violations
  useKeyboardShortcut('1', () => isAuthenticated && startTransition(() => setSelectedTab(0)), { ctrl: true }); // Ctrl+1: Dashboard
  useKeyboardShortcut('2', () => isAuthenticated && startTransition(() => setSelectedTab(1)), { ctrl: true }); // Ctrl+2: Sales
  useKeyboardShortcut('3', () => isAuthenticated && startTransition(() => setSelectedTab(2)), { ctrl: true }); // Ctrl+3: Customers
  useKeyboardShortcut('4', () => isAuthenticated && startTransition(() => setSelectedTab(3)), { ctrl: true }); // Ctrl+4: Suppliers
  useKeyboardShortcut('5', () => isAuthenticated && startTransition(() => setSelectedTab(4)), { ctrl: true }); // Ctrl+5: Inventory
